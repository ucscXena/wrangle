...
for (( i=0; i<$numdir+$numPerRUN-1; i=i+$numPerRUN )) do
  ...
  for (( j=0; j<$numPerRUN; j++ )) do
	 ...
	 if [[ -d $dir ]]; then
	     ...
	     bundle_uuid=$(basename $dir) # HCA data has duplicate cell suspension id weird, so use subdir/bundle_uuid for now
	     command="$command nice python $xenaH5Dir/h5_xena.py $input_h5 GRCh38 $output $bundle_uuid ; "
	 fi
     fi
  done
  if [[ $command != '' ]]; then
